...
export const AuthProvider = ({ children }) => {
  ...
  const googleLogin = async (googleUser) => {
    ...
    const existingUser = db[googleEmail];

    const googleAppUser = existingUser || {
    ...
    db[googleEmail] = googleAppUser;
    db[googleAppUser.username] = googleAppUser;

    await AsyncStorage.setItem(USERS_DB_KEY, JSON.stringify(db));
    await AsyncStorage.setItem(CURRENT_SESSION_KEY, JSON.stringify(googleAppUser));

    setUser(googleAppUser);
